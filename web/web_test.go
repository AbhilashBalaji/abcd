...
func createShardDb(t *testing.T, idx int) *db.Database {
	...
	if err != nil {
		t.Fatalf("could not create temp db &d: %v", err)
	}
	defer tmpFile.Close()
	name := tmpFile.Name()
...
func TestWebServer(t *testing.T) {
	...
	value1, err := db1.GetKey("bar")
	if err != nil {
		t.Fatalf("bar key error: %v", err)
	}

	want1 := "value-bar"
	if !bytes.Equal(value1, []byte(want1)) {
		t.Errorf("Unexpected value of bar key: got %q, want %q", value1, want1)
	}

	value2, err := db2.GetKey("foo")
	if err != nil {
		t.Fatalf("foo key error: %v", err)
	}

	want2 := "value-foo"
	if !bytes.Equal(value2, []byte(want2)) {
		t.Errorf("Unexpected value of foo key: got %q, want %q", value2, want2)
	}
}
